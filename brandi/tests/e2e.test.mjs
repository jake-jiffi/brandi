...
describe('a fully specified brand', () => {
  ...
  test('the brand book carries the real content, not placeholders', async () => {
    ...
    // disk, so no colourway can be painted either, no co-branding rule, no
    // favicon artwork, no pillar "why"), so the rule is now: every placeholder
    // names one of those, and nothing else.
    const placeholders = [...html.matchAll(/\[Not recorded yet: ([^\]]+)\]/g)].map((m) => m[1]);
    for (const p of placeholders) {
      assert.match(p, /logo artwork|an approved colourway|co-branding|favicon|why this pillar|story behind the mark|coated match|uncoated match|usage rights|price or opening hours|proof artboards/, `unexpected placeholder: ${p}`);
    }
  });
